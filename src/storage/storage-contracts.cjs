...
function assertManagedMediaId(value) {
  if (typeof value !== 'string' || !UUID_PATTERN.test(value)) {
    throw new TypeError('managed media id is invalid.');
  }
  return value;
}

function normaliseSensitiveKey(key) {
...
function assertSafeJsonValue(value, label = 'value', depth = 0) {
  ...
  if (Buffer.isBuffer(value) || ArrayBuffer.isView(value)) {
    throw new TypeError(`${label} must not contain binary data.`);
  }
  ...
  if (Array.isArray(value)) {
    if (value.length > MAX_JSON_ARRAY_LENGTH) throw new TypeError(`${label} contains too many items.`);
    value.forEach((item, index) => assertSafeJsonValue(item, `${label}[${index}]`, depth + 1));
    return;
  }
...
function validateMediaIds(value) {
  ...
  for (const mediaId of value) {
    ...
    if (unique.has(mediaId)) throw new TypeError('mediaIds must not contain duplicates.');
    unique.add(mediaId);
  }
  return [...value];
}
...
function validateMediaRecord(media) {
  ...
  if (
    ...
    media.originalFilename.length > 255 ||
    /[\\/\0]/.test(media.originalFilename)
  ) {
    throw new TypeError('originalFilename is invalid.');
  }
  if (
    typeof media.managedReference !== 'string' ||
    !MANAGED_REFERENCE_PATTERN.test(media.managedReference) ||
    !media.managedReference.includes(media.id)
  ) {
    throw new TypeError('managedReference is invalid.');
  }
  if (!Number.isSafeInteger(media.fileSize) || media.fileSize <= 0 || media.fileSize > MAX_MEDIA_FILE_SIZE) {
    throw new TypeError('media fileSize is invalid.');
  }
  if (typeof media.sha256 !== 'string' || !SHA256_PATTERN.test(media.sha256)) {
    throw new TypeError('media sha256 is invalid.');
  }
  ...
  if (
    ...
    (typeof media.durationSeconds !== 'number' ||
      ...
      media.durationSeconds > 86400)
  ) {
    throw new TypeError('media durationSeconds is invalid.');
  }
  for (const field of ['container', 'codec']) {
    if (media[field] !== null && (typeof media[field] !== 'string' || media[field].length > 64)) {
      throw new TypeError(`media ${field} is invalid.`);
    }
  }
...
function validateApplicationState(application) {
  assertExactKeys(
    application,
    ['schemaVersion', 'settings', 'selectedProjectId', 'recentProjectIds'],
    'application state'
  );
  if (application.schemaVersion !== APPLICATION_SCHEMA_VERSION) {
    throw new TypeError('Unsupported application-state schema version.');
  }
  ...
  if (!Array.isArray(application.recentProjectIds) || application.recentProjectIds.length > RECENT_PROJECT_LIMIT) {
    throw new TypeError('recentProjectIds must be a bounded array.');
  }
  const recent = new Set();
  for (const projectId of application.recentProjectIds) {
    assertProjectId(projectId, 'recentProjectId');
    if (recent.has(projectId)) throw new TypeError('recentProjectIds must not contain duplicates.');
...
function validateProject(project) {
  assertExactKeys(
    project,
    ['id', 'schemaVersion', 'title', 'status', 'mediaIds', 'extensions', 'createdAt', 'updatedAt', 'revision'],
    'project'
  );
  ...
  if (typeof project.title !== 'string' || project.title.length === 0 || project.title.length > PROJECT_TITLE_LIMIT) {
...
function validateDocument(document) {
  ...
  for (const [projectId, project] of Object.entries(document.projects)) {
    ...
    if (project.id !== projectId) throw new TypeError('project key does not match project id.');
  }
  if (document.media !== undefined) {
    if (!isPlainObject(document.media)) throw new TypeError('media must be an object.');
    const hashes = new Set();
    for (const [mediaId, media] of Object.entries(document.media)) {
      assertManagedMediaId(mediaId);
      validateMediaRecord(media);
      if (media.id !== mediaId) throw new TypeError('media key does not match media id.');
      if (hashes.has(media.sha256)) throw new TypeError('media content hashes must be unique.');
      hashes.add(media.sha256);
    }
  }
  const knownProjectIds = new Set(Object.keys(document.projects));
  if (document.application.selectedProjectId && !knownProjectIds.has(document.application.selectedProjectId)) {
    throw new TypeError('selectedProjectId does not reference a known project.');
  }
  for (const projectId of document.application.recentProjectIds) {
    if (!knownProjectIds.has(projectId)) throw new TypeError('recentProjectIds contains an unknown project.');
  }
  return document;
}

function validateApplicationUpdateRequest(value) {
  assertExactKeys(value, ['expectedRevision', 'patch'], 'application update request');
  ...
  if ('settings' in value.patch) {
    if (!isPlainObject(value.patch.settings)) throw new TypeError('settings must be an object.');
    assertSafeJsonValue(value.patch.settings, 'settings');
  }
  if ('selectedProjectId' in value.patch && value.patch.selectedProjectId !== null) {
    assertProjectId(value.patch.selectedProjectId, 'selectedProjectId');
  }
  if ('recentProjectIds' in value.patch) {
    if (!Array.isArray(value.patch.recentProjectIds) || value.patch.recentProjectIds.length > RECENT_PROJECT_LIMIT) {
      throw new TypeError('recentProjectIds must be a bounded array.');
    }
    const seen = new Set();
    for (const projectId of value.patch.recentProjectIds) {
      assertProjectId(projectId, 'recentProjectId');
      if (seen.has(projectId)) throw new TypeError('recentProjectIds must not contain duplicates.');
      seen.add(projectId);
    }
  }
  return value;
...
function validateCreateProjectRequest(value) {
  assertExactKeys(value, ['expectedRevision', 'title', 'mediaIds', 'extensions'], 'project create request');
  assertRevision(value.expectedRevision, 'expectedRevision');
  if (typeof value.title !== 'string' || value.title.length === 0 || value.title.length > PROJECT_TITLE_LIMIT) {
    throw new TypeError('project title is invalid.');
  }
  if ('mediaIds' in value) validateMediaIds(value.mediaIds);
  if ('extensions' in value) {
    if (!isPlainObject(value.extensions)) throw new TypeError('extensions must be an object.');
    assertSafeJsonValue(value.extensions, 'extensions');
  }
  return value;
}

function validateRendererCreateProjectRequest(value) {
  validateCreateProjectRequest(value);
  if ('mediaIds' in value) throw new TypeError('Renderer project creation must attach media through the media service.');
  return value;
}

function validateProjectReadRequest(value) {
  assertExactKeys(value, ['projectId'], 'project read request');
  assertProjectId(value.projectId);
  return value;
}

function validateUpdateProjectRequest(value) {
  assertExactKeys(value, ['projectId', 'expectedRevision', 'patch'], 'project update request');
  assertProjectId(value.projectId);
  assertRevision(value.expectedRevision, 'expectedRevision');
  assertExactKeys(value.patch, ['title', 'mediaIds', 'extensions'], 'project patch');
  if ('title' in value.patch) {
    if (typeof value.patch.title !== 'string' || value.patch.title.length === 0 || value.patch.title.length > PROJECT_TITLE_LIMIT) {
      throw new TypeError('project title is invalid.');
    }
  }
  if ('mediaIds' in value.patch) validateMediaIds(value.patch.mediaIds);
  if ('extensions' in value.patch) {
    if (!isPlainObject(value.patch.extensions)) throw new TypeError('extensions must be an object.');
    assertSafeJsonValue(value.patch.extensions, 'extensions');
  }
  return value;
}

function validateRendererUpdateProjectRequest(value) {
  validateUpdateProjectRequest(value);
  if ('mediaIds' in value.patch) throw new TypeError('Renderer project updates must attach media through the media service.');
  return value;
}

function validateArchiveProjectRequest(value) {
  assertExactKeys(value, ['projectId', 'expectedRevision'], 'project archive request');
  assertProjectId(value.projectId);
  assertRevision(value.expectedRevision, 'expectedRevision');
  return value;
}
